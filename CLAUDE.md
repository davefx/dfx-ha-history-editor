...
- `recalculate_short_term_stat` — rebuilds one 5-min row from the numeric states in that window. If the window is now empty (all records deleted), it carries forward the last prior numeric value rather than leaving a gap. Use it only for a **single** period; every multi-period caller must go through `recalculate_short_term_stats_range`.
- `recalculate_short_term_stats_range` — recalculates a batch of 5-min periods and applies the sum cascade **once** for the whole range. Each row is adjusted by its own delta; only rows *after* the range are shifted, by the delta of the last period in it. Cascading per period instead made a range of N periods shift later rows by the sum of N deltas, which drove running totals negative (issue #76).
...
- `_cascade_sum_adjustment` — for `state_class=total` / `total_increasing` sensors, the `sum` column is a running total. When the last value in a period changes by Δ, every subsequent short-term and long-term row must have Δ added to its `sum`. Call it **once per contiguous range**, never once per period — see `recalculate_short_term_stats_range`. Uses `synchronize_session=False` bulk updates for speed; the caller must `session.flush()` and `session.expire_all()` (or `session.commit()` + `session.expire_all()`) before reading those rows again.
...
Ordering invariant in `update_statistics_after_state_change`: short-term periods are processed in chronological order (later periods read sums already adjusted by earlier ones), then `session.flush()` + `session.expire_all()`, *then* long-term periods. Breaking this order causes long-term rows to be built from stale cached short-term rows.
...
The source-data guard (`_check_source_data_blocks_edit`) is the single implementation behind both the singular and the bulk stats paths. A short-term row is blocked while state history exists in its 5-min window; a long-term row is blocked while state history exists in its hour — **not** when short-term rows exist, since those are derived data that outlive their states and blocking on them left users unable to repair an entity at all (issue #76).
